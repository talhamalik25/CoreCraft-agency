...
const Contact = () => {
  ...
  return (
    <div ref={portalRef} className="bg-black px-4 pb-20 pt-28 sm:px-6 md:px-20 md:pb-40 md:pt-36">
      <div className="mx-auto grid max-w-7xl grid-cols-1 border border-zinc-800 lg:grid-cols-[40%_60%]">
        <aside className="border-b border-zinc-800 p-7 sm:p-10 lg:border-b-0 lg:border-r lg:p-14">
          <div data-form-line>
            <div className="mb-6 font-mono text-[10px] uppercase tracking-[0.22em] text-teal">Get in touch / 01</div>
          ...
          <div data-form-line className="mt-16 border-t border-zinc-800 pt-6">
          ...
          <div data-form-line className="mt-8 grid grid-cols-1 border border-zinc-800 sm:grid-cols-2 lg:grid-cols-1">
            <a href="mailto:corecraftagency07@gmail.com" className="group border-b border-zinc-800 p-5 transition-colors hover:bg-card sm:border-b-0 sm:border-r lg:border-b lg:border-r-0">
          ...
          <div data-form-line className="mt-8 flex gap-5 border-t border-zinc-800 pt-5">
            <a href="https://www.linkedin.com/company/corecraftagency/" target="_blank" rel="noopener noreferrer" aria-label="CoreCraft on LinkedIn" className="text-white/50 transition-colors hover:text-teal"><Linkedin size={18} /></a>
            <a href="https://www.instagram.com/corecraftagency/" target="_blank" rel="noopener noreferrer" aria-label="CoreCraft on Instagram" className="text-white/50 transition-colors hover:text-teal"><Instagram size={18} /></a>
            <ArrowUpRight className="ml-auto text-teal" size={18} />
          </div>
        </aside>

        <section className="p-7 sm:p-10 md:p-14">
          <div data-form-line className="mb-12 flex items-end justify-between gap-6 border-b border-zinc-800 pb-6">
            <div>
              <div className="mb-4 font-mono text-[10px] uppercase tracking-[0.22em] text-teal">Project intake / 02</div>
          ...
          <form onSubmit={onSubmit} className="space-y-9">
            <fieldset data-form-line>
              ...
              <div className="flex flex-wrap gap-2">
                {PROJECT_TYPES.map((type) => {
                  ...
                  return (
                    <button key={type} type="button" aria-pressed={isSelected} onClick={() => toggleProjectType(type)} className={`rounded-full border px-4 py-2.5 font-dm text-xs transition-all duration-300 ${isSelected ? 'border-teal bg-teal text-black shadow-[0_0_22px_rgba(0,168,150,0.24)]' : 'border-zinc-800 text-white/65 hover:border-teal/60 hover:text-white'}`}>
                      {type}
                    </button>
                  );
                })}
            ...
            <div data-form-line className="group border-b border-zinc-800 transition-all duration-300 focus-within:border-teal focus-within:shadow-[0_12px_24px_-20px_rgba(0,168,150,0.95)]">
            ...
            <div data-form-line className="flex flex-col items-start gap-5 pt-3 sm:flex-row sm:items-center">
              <button type="submit" disabled={isSending} className="inline-flex w-full items-center justify-center gap-3 rounded-full bg-teal px-8 py-4 font-dm text-xs uppercase tracking-[0.2em] text-black shadow-[0_0_30px_rgba(0,168,150,0.22)] transition-all duration-300 hover:bg-white disabled:cursor-not-allowed disabled:opacity-50 sm:w-auto">
                {isSending ? 'Sending...' : 'Send inquiry'}
                <ArrowUpRight size={16} />
              </button>
              {result && <p className={`font-dm text-xs uppercase tracking-widest ${result.includes('Success') ? 'text-teal' : 'text-red-500'}`}>{result}</p>}
            </div>
          </form>
        </section>
      </div>
...
const Field = ({ label, name, type = 'text', placeholder, required = false }) => (
  <div className="group border-b border-zinc-800 transition-all duration-300 focus-within:border-teal focus-within:shadow-[0_12px_24px_-20px_rgba(0,168,150,0.95)]">
